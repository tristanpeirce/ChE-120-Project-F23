...
import time # imports the time module for the ability to delay the program from quitting (TP)

def instructions(): # function to print the instructions that inform the players on how to play the game (WC)
    '''
    () -> NoneType
...
\nHow To Play:
    ...
    ''')
    return None # the function returns nothing, the only thing that appears when this function is run is the print statement above (WC)

def valid_int_input(num): # function for validating the input of how many players are playing (WC)
    '''
    (int) -> bool
    
    This function takes an integer input and returns True if the input is numeric and in between 2 and 6, or returns False otherwise (WC).
    '''
    if num.isnumeric() and (num <= '6' and num >= '2'): # if the input is a number between 2 and 6, the function returns True(WC)
        return True
    else: # if the input is anything else other than a number between 2 and 6, the function returns False (WC)
        return False
    
def roll(player): # function for rolling the die (AD)
    '''
    (int) -> int
    ...
    rolled = random.randint(1,6) # generates a random integer between 1 and 6 to represent the rolling of the die, assigns this generated value to variable 'rolled' (AD)
    ...
    if rolled == 1: # if the number assigned to rolled is 1, the player is told their turn is over, and the game continues (AD)
    ...
    else: # if the number assigned to rolled is anything but a 1, the player is informed of the die rolled, and the players turn can continue if they wish (AD)
...
def get_total(player): # function that takes an input of the player number and gets their total score based on their dice rolls (WC)
    '''
    (int) -> int
    ...
    for i in players[player]: # initiates a for loop that will add every number of each die rolled to the 'total' variable (WC)
        total += i # adds the numbers rolled to the 'total' variable (WC)
...
instructions() # displays instructions from the 'instructions' function (AA)
...
if valid_int_input(num_players): # ensures that the number of players is valid given the parameters (AA)
...
while playing: # initializes the start of the game that will continue until someone has won or the game has been quit (TP)
    ...
    while True: # loop that allows the player to keep rolling over and over again until a 1 is rolled, they quit the game, or decide not to roll again (TP)
        if roll_question.lower() == 'quit': # if the player wishes to quit the game, a quitting message is printed and after a delay, the program is quit (TP)
            ...
            time.sleep(1)
            sys.exit()
        elif roll_question.lower() == 'y': # if the player wishes to roll the die, the 'roll' function is used to evaluate the value of their roll and whether they can continue or not (TP)
